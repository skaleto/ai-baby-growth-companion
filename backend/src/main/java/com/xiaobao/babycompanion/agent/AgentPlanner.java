...
public class AgentPlanner {
    ...
    private static final String PLANNER_SYSTEM_PROMPT = """
            ...
            输出必须是严格 JSON 对象，schema:
            {
              ...
              "topics": ["feeding|sleep|poop|temperature|vaccine|growth|memory|reminder|policy|general"],
    ...
    public AgentPlan heuristic(AgentChatRequest request, RecordSignals signals) {
        ...
        boolean needsWeb = message.matches(".*(查|查询|搜|搜索|联网|最新|政策|规定|官方|通知|天气|哪里|地址|电话|办理|流程).*");
        boolean reminder = message.matches(".*(提醒|闹钟|记得|定时|每隔\\s*[\\d一二两三四五六七八九十半]+\\s*(分钟|分|小时)).*");
        boolean record = signals.concreteCareLog() ||
                signals.topics().stream().anyMatch((topic) -> List.of("feeding", "sleep", "poop", "temperature", "growth", "memory").contains(topic));
