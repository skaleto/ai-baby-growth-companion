...
const overrides = new Map([
  ...
  ["Planner + Runtime", { status: "covered", coverage: [layer("l0_l1", "npm run test:agent-benchmark covers planner/runtime/policy behavior."), l2(["daily-observation-context", "expense-record", "qa-care"], "L2 covers record, ledger, and data-linked companion paths.")] }],
  ...
  ["安全边界", { status: "covered", coverage: [layer("l0_l1", "AgentBenchmarkTests cover fever, medicine, vaccine, caregiver high-risk, and medical-safety boundaries."), l2(["fever-risk", "safety-refuse"], "L2 covers high-risk health copy without creating AI reminder/todo mutations.")] }],
  ...
  ["聊天图片/视频上传", { status: "covered_by_layer", coverage: [layer("frontend", "verify:frontend covers attachment UI compatibility."), l2(["screenshot-ignore"], "L2 keeps visual attachment handling read-only unless a retained record/ledger tool has explicit text fields.")] }],
  ...
  ["自动准入", { status: "covered_by_layer", coverage: [l2(["screenshot-ignore"], "L2 asserts visual inputs do not mutate state by default."), layer("l0_l1", "scripts/test-album-domain.mjs covers album-domain classification rules while agent save_to_album is disabled.")] }],
  ...
  ["Agent 创建提醒", { status: "covered_by_layer", coverage: [layer("backend", "capability-manifest disables set_reminder and the runtime prompt excludes AI reminder/todo tools."), layer("l0_l1", "AgentBenchmarkTests cover that reminder requests do not become direct agent writes.")] }],
  ...
  ["Doubao 模型", { status: "covered_by_layer", coverage: [layer("backend", "Agent model routing and runtime tests cover Doubao multimodal/low-latency handling."), l2(["screenshot-ignore"], "Image L2 scenarios exercise the read-only multimodal attachment path.")] }],

  ["Tool-first 喂养写入", { status: "covered", coverage: [layer("backend", "RecordFeedingEventToolTests and AgentRuntimeToolFirstContractTests cover applied careLog writes via actionResults."), l2(["feed-complete", "feed-mixed-missing-type"], "L2 covers applied feeding writes and missing feeding-type no-write behavior.")] }],
  ["Tool-first 成长待确认", { status: "covered", coverage: [layer("backend", "AgentActionToolsTests and AgentMutationServiceTests cover growth measurement pendingEffects creation."), l2(["growth-measurement-complete", "growth-measurement-ambiguous-unit", "growth-measurement-out-of-range"], "L2 covers persisted growth pending drafts and no-write boundaries.")] }],
  ["Tool-first 账本待确认", { status: "covered", coverage: [layer("backend", "AgentActionToolsTests and AgentRuntimeTests cover text/image expense pendingEffects without direct expense writes."), l2(["expense-record"], "L2 covers persisted ledger pending drafts before confirmation.")] }],
  ["最终回复基于工具结果", { status: "covered", coverage: [layer("backend", "AgentActionResponseGuardTests cover false 已记录/待确认 claims without actionResults."), layer("l0_l1", "AgentBenchmarkTests cover response-guard regressions for applied, pending_created, and needs_input outcomes.")] }],
  ["不提供 AI 提醒工具", { status: "covered", coverage: [layer("backend", "capability-manifest requires set_reminder disabled and prompt rules route reminder requests to manual My-page management."), layer("l0_l1", "AgentBenchmarkTests cover no AI reminder write tool is exposed.")] }],
