...
| P1 | 图片/小票识别权益 | 后续 Pro 可用图片/小票识别，识别结果必须待确认。 | 当前计划只保留接口边界，不在 P0 做完整实现。 |
...
| P1 | Pro/权益说明 | 按模块说明图片/视频理解、小票识别、批量整理等权益，不暴露模型设置。 | 我的页不出现模型选择、深度思考、快速模式设置。 |
...
| P0 | 图片/视频入口权益 | 未开通 Pro 时相机置灰但可点说明；开通后可选择媒体。 | frontend smoke + Pro fixture。 |
