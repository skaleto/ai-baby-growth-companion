...
| P0 | 单条记录 upsert/delete | `PUT /api/app/state/{collection}/{id}`、`DELETE` 支持 profile/messages/growth/care/reminder/memory/pending/album/expenses/summary。 | API 逐集合写入/读取/删除，断言共享或私有边界正确。 |
...
| 优先级 | 功能 | 当前实现 | 自动化验证建议 |
| --- | --- | --- | --- |
| P0 | 提醒列表 | 分组：今天要做、已逾期、已完成；不显示未来安排分组。 | 构造今日/过期/完成提醒，断言分组数量和文案。 |
...
| P1 | 顺延 | 非循环顺延一天；循环或闹铃顺延 30 分钟。 | 点击顺延，断言 dueAt 和 history 更新。 |
...
### 14. OpenAI/DeepSeek/Doubao 模型与配置能力
