...
目标：手动记录主线永久免费；凡走 AI 助手的整理都属 Pro，Free 每月有固定次数免费体验，用完引导申请内测。
...
| 图片 / 视频 AI 理解 | 每月免费额度内可用 | 不限次 | 凡走 AI 助手即 Pro 能力 |
| 账本小票 / 图片 AI 识别 | 每月免费额度内可用 | 不限次 | 同上 |
| 高级趋势洞察 | 每月免费额度内可用 | 不限次 | 不做同龄排名 |
...
| 维度 | 默认值 | 口径 |
| --- | --- | --- |
| 每月免费 AI 次数（Free） | 每家庭每月 10 次 | 只数顶层用户回合（`agent_chat` / `agent_stream`），planner/视觉/记账/压缩等子步不计；可配 `app.pro.free-monthly-ai-quota` |
| Pro 家庭 | 不限次 | family entitlement 有效即放行 |
| 月 token 预算 | 每家庭月 200 万 token | `AgentRequestGuard` 既有保护，超出即拦 |
| 每分钟限流 | 每家庭 30 次/分钟 | 既有防滥用 |
...
- Free 用户每月免费额度用尽后，任意 AI 助手调用（含纯文字、图片/视频整理、账本 AI）返回 403 PRO_QUOTA_EXCEEDED，前端展示「申请 Pro 内测」入口并提示本月剩余次数。
- 文案锚点：凡走 AI 助手即 Pro；Free 每月有固定次数免费体验，用完引导申请内测。
...
- Pro 描述：凡走 AI 助手即 Pro，Free 每月若干次免费体验，用完引导申请内测。
- 不写电商、专家、社区、预测排名。
