...
| `frontend-001` | covered by non-agent gate | `npm run verify:frontend` and `docs/frontend-verification.md`. | UI changes need fresh evidence; build alone is not enough. |
...
| `mobile-001` | known gap | `scripts/native-capability-audit.mjs` and `docs/native-capability-benchmark.md` track static evidence. | Add real device probes for OS-level input/delivery/apply behavior. |
