...
## Purpose
Define the project-level development workflow requirements that make local build checks, browser smoke testing, mobile viewport observation, native-risk escalation, and explicit verification handoff part of Codex frontend delivery.
...
The development workflow SHALL use a default mobile viewport matrix covering representative compact iPhone, mainstream iPhone, large iPhone, compact Android, mainstream Android, and large Android classes for UI and interaction changes.

#### Scenario: Responsive visual check runs
- **WHEN** Codex verifies a UI or interaction change in the browser
- **THEN** Codex MUST inspect the affected page or flow at `375x667`, `390x844`, `430x932`, `360x800`, `412x915`, and `432x960`
...
#### Scenario: Keyboard-sensitive UI elements are changed
- **WHEN** the change affects mobile forms, text inputs, composers, modals, drawers, or fixed panels
- **THEN** Codex MUST exercise an input focus with simulated keyboard viewport shrink, confirm the focused control remains visible while the keyboard is open, then exercise keyboard close/viewport restore, or report why that path cannot be automated and what real-device validation remains
